...
1. 将 `moon.mod` 中的 `repository` 替换为真实公开仓库地址。
2. 将模块 owner 替换为你的 Mooncakes 用户名。
3. 将 GitHub 仓库同步到 Gitlink。
4. 保持 10-20 个有效 commits，避免空提交、重复提交或无意义拆分。
5. 登录 Mooncakes 后执行 `moon publish`。
